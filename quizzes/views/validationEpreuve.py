...
def list_validations(request):
    ...
    # Pagination : 1 éléments par page
    paginator = Paginator(list_validations, 5)
    try:
        page = request.GET.get("page")
        if not page:
            page = 1
        list_validations = paginator.page(page)

    except EmptyPage:
        # si on dépasse la limite des pages on prends la dernière
        list_validations = paginator.page(paginator.num_pages())
...
def new_response(request):
    if request.method =='POST':
        formulaire = ReponseForm(request.POST, request.FILES)
    ...
    return render(request,'u_quizzes/validationEpreuve/reponseForm.html', context)